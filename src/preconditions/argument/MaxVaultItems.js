...
class MaxVaultItems extends patron.ArgumentPrecondition {
  ...
  async run(command, msg, argument, args, value, options) {
    ...
    if (items.length >= options.maxUnique) {
    ...
    } else if (value > options.maxAmount || gang.vault[args.item.names[0]] !== undefined && gang.vault[args.item.names[0]] + value > options.maxAmount) {
